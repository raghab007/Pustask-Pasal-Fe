...
const NewReleases = () => {
  ...
  useEffect(() => {
    const fetchNewReleases = async () => {
      try {
        const response = await axios.get("http://localhost:5001/api/Books/new-releases");
        setNewReleases(response.data.books);
        setLoading(false);
      } catch (err) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-lg text-gray-600">
            Explore our latest book releases
          </p>
        </div>
            ...
            >
              <div className="relative h-[300px] w-full">
                ...
                <div className="absolute top-2 right-2">
                  <div className="bg-gradient-to-r from-purple-500 to-pink-500 text-white px-2 py-1 rounded-full text-sm font-semibold flex items-center">
        ...
        {newReleases.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No new releases available at the moment.</p>
          </div>
        )}
      </div>
    </div>
