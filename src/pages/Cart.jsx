...
import { getCart, updateCartItem, removeCartItem, clearCart } from "../utils/cartUtils";
  ...
  if (!cart || cart.items.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center">
        ...
        <p className="text-gray-600 mb-6">Add some books to your cart to continue shopping</p>
        <button
              ...
              <div className="divide-y divide-gray-200">
                {cart.items.map((item) => (
                  <div key={item.id} className="p-6 flex items-center">
                    <img
                      src={`http://localhost:5001/api/Images/Books/${item.imageUrl}`}
                      alt={item.title}
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900">Rs {cart.summary.subtotal.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Total Discount</span>
                  <span className="text-gray-900">Rs {cart.summary.totalDiscount.toFixed(2)}</span>
                </div>
                <div className="border-t border-gray-200 pt-4">
                  <div className="flex justify-between">
                    <span className="text-lg font-medium text-gray-900">Total</span>
                    <span className="text-lg font-medium text-gray-900">
                      Rs {cart.summary.grandTotal.toFixed(2)}
                    </span>
                  </div>
                </div>
                <button
